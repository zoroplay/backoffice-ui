...
import { onlineSalesColumns } from "./columns";
import { OnlineSalesTypes } from "./columns";
...
function OnlineSales() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await reportsApi.getOnlineSales(payload);

      
      if (res && typeof res === "object" && "data" in res) {
        const payloadData = (res as any).data;
        const rows = Array.isArray(payloadData.data) ? payloadData.data : [];
        setFilteredData(rows as OnlineSalesTypes[]);
      } else if (Array.isArray(res)) {
        setFilteredData(res as OnlineSalesTypes[]);
      } else {
        setFilteredData([]);
      }
    } catch (err) {
      const apiErr = normalizeApiError(err);
      setError(apiErr.message ?? "Failed to fetch online sales");
      setFilteredData([]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <section className="space-y-6 p-4">
      {/* Breadcrumb */}
      <PageBreadcrumb pageTitle="Online Sales" />
      ...
      {isLoading ? (
        ...
        <>
          ...
          <DataTable columns={onlineSalesColumns} data={filteredData} />
        </>
      )}
    </section>
  );
...
export default withAuth(OnlineSales);
