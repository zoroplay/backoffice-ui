...
function mapRow(itemValue: unknown, index: number): AccountingRow {
  const item = asRecord(itemValue);
  return {
    id: String(item.id ?? item.user_id ?? item.userId ?? item.username ?? `accounting-${index}`),
    ...
    channel: (item.channel ?? item.client_type ?? item.clientType ?? "-") as AccountingRow["channel"],
    totalStake: toNumber(item.total_stake ?? item.totalStake),
    totalGain: toNumber(item.total_gain ?? item.totalGain ?? item.totalWinnings),
    companyProfit: toNumber(item.company_profit ?? item.companyProfit),
...
function paginationFrom(data: unknown, page: number, count: number): Pagination {
  ...
  const perPage = toNumber(payload.per_page ?? payload.perPage ?? bets.per_page ?? bets.perPage, count || 10);
  const total = toNumber(payload.total ?? payload.count ?? bets.total, count);
  const currentPage = toNumber(payload.current_page ?? payload.currentPage ?? bets.current_page ?? bets.currentPage, page);

  return {
    total,
    per_page: perPage,
    from: toNumber(payload.from ?? bets.from, total ? (currentPage - 1) * perPage + 1 : 0),
    to: toNumber(payload.to ?? bets.to, Math.min(currentPage * perPage, total)),
    current_page: currentPage,
    last_page: toNumber(payload.last_page ?? payload.lastPage ?? bets.last_page ?? bets.lastPage, perPage ? Math.max(1, Math.ceil(total / perPage)) : 1),
  };
}
...
export default function AccountingReportClient() {
  ...
  const [totals, setTotals] = useState<Totals>({ stake: 0, winnings: 0, profit: 0, balance: 0 });
  ...
  async function search(page = 1) {
    ...
    const response = await POSTREQUEST<any>(`api/admin/reporting/get-accounting-report?page=${page}`, payload);
    setLoading(false);
    ...
    const rawRows = Array.isArray(bets.data) ? bets.data : Array.isArray(body.data) ? body.data : [];
    const mapped = rawRows.map(mapRow);

    setRows(mapped);
    setTotals({
      stake: toNumber(body.totalStake, mapped.reduce((sum, row) => sum + row.totalStake, 0)),
      winnings: toNumber(body.totalWinnings, mapped.reduce((sum, row) => sum + row.totalGain, 0)),
      profit: toNumber(body.totalCoProfit, mapped.reduce((sum, row) => sum + row.companyProfit, 0)),
      balance: toNumber(body.totalBalance, mapped.reduce((sum, row) => sum + row.balance, 0)),
    });
  ...
  function resetFilter() {
    ...
    setPaging(true);
    setRows([]);
    setTotals({ stake: 0, winnings: 0, profit: 0, balance: 0 });
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-950">
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void search(1); }}>
          ...
          <div className="rounded-md bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:bg-white/[0.03] dark:text-gray-400">
            POST api/admin/reporting/get-accounting-report?page={pagination.current_page} with period, from, to, client_type, changed balances, and paging.
          </div>
        </form>
      </section>
      ...
      <section className="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-950">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-900">
              ...
              {!rows.length ? <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-500">{loading ? "Loading report" : "No record found"}</td></tr> : null}
            </tbody>
        ...
        {paging ? (
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 px-5 py-4 text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400">
            <span>Showing {pagination.total ? `${pagination.from} to ${pagination.to} of ${pagination.total}` : rows.length} rows</span>
